...
def planning(Age,BW,BMI,Alb,T0,T1,T2,T3,T4,T5,T6.T7):
    
    #expert data for normalization
    data_expert = pd.read_csv(filepath2)#expert data for normalization
    ...
    #patient data
    input1 = np.array([Age,BW,BMI,Alb,T0])
    ...
    input_scaler = np.concatenate([input1_scaler, input], 1)
    #loading
    network_loading = TwoLayerNet(input_size=input_scaler_size, hidden_size=15, output_size=len(ll))
    ...
    pred_loading_one_hot_to_index = np.argmax(network_loading.predict(parameter_scaler),axis=1)
    pred_loading_dose = ll[pred_loading_one_hot_to_index]
    
    #maintenance
    network_maintenance = TwoLayerNet(input_size=input_scaler_size, hidden_size=15, output_size=len(lm))
    ...
    pred_maintenance_one_hot_to_index = np.argmax(network_maintenance.predict(parameter_scaler),axis=1)
